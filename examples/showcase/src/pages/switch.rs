...
pub fn page(scheduler: Arc<Mutex<AnimationScheduler>>) -> Box<dyn Widget> {
    ...
    Box::new(rust_ui::column![
        ...
        rust_ui::row![
            rust_ui::column![
                switch("Small", true).size(SwitchSize::Sm).scheduler(sched5.clone()),
                text("Sm").size(11.0).color(Color::hex("#555870")),
            ].spacing(4.0),
            rust_ui::column![
                switch("Medium (default)", true).size(SwitchSize::Md).scheduler(sched6.clone()),
                text("Md").size(11.0).color(Color::hex("#555870")),
            ].spacing(4.0),
            rust_ui::column![
                switch("Large", true).size(SwitchSize::Lg).scheduler(sched7.clone()),
