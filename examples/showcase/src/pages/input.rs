...
pub fn page() -> Box<dyn Widget> {
    Box::new(rust_ui::column![
        ...
        rust_ui::row![
            ...
            rust_ui::column![
                input("").placeholder("Disabled").disabled(true).width(180.0),
        ...
        rust_ui::column![
            text("← →  move cursor      Home / End  jump to edge").size(12.0).color(Color::hex("#8b8fa8")),
            text("Shift+← →  extend selection      Ctrl+A  select all").size(12.0).color(Color::hex("#8b8fa8")),
            text("Ctrl+C  copy      Ctrl+X  cut      Ctrl+V  paste      Del  delete forward").size(12.0).color(Color::hex("#8b8fa8")),
        ].spacing(4.0),
    ].spacing(14.0))
}
