...
impl Row {
    fn child_rects(&self, bounds: Rect, theme: &Theme) -> Vec<Rect> {
        ...
        let mut sizes: Vec<(f32, f32)> = self
            .children
            .iter()
...
impl Widget for Row {
    ...
    fn handle_event(&mut self, event: &Event, bounds: Rect) -> EventStatus {
        ...
        let rects = self.child_rects(bounds, &theme);
        match event {
            // Mouse move is broadcast so widgets can update hover state.
            Event::MouseMove { .. } => {
            ...
            // Mouse down/up is broadcast so widgets can update their own focus state
            // (e.g. an Input can unfocus itself when clicked outside).
            ...
            // Pointer events that have a position are hit-tested so only the target
            // child receives them. If ignored, we allow it to bubble to the next
            // sibling under the same point (rare but useful for overlays).
            ...
            // Keyboard / text / focus events — stop at first consumer.
            _ => {
                for (child, cb) in self.children.iter_mut().zip(rects.iter()) {
                    if child.handle_event(event, *cb) == EventStatus::Consumed {
                        return EventStatus::Consumed;
