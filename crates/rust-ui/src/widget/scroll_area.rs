...
impl ScrollArea {
    ...
    fn viewport(&self, bounds: Rect) -> Rect {
        let w = self.max_width.map_or(bounds.width, |mw| mw.min(bounds.width));
        let h = self.max_height.map_or(bounds.height, |mh| mh.min(bounds.height));
    ...
    fn thumb_rect_v(&self, vp: &Rect, content_h: f32, scroll_y: f32) -> Option<Rect> {
        if content_h <= vp.height { return None; }
    ...
    fn thumb_rect_h(&self, vp: &Rect, content_w: f32, scroll_x: f32) -> Option<Rect> {
        if content_w <= vp.width { return None; }
    ...
    fn draw_scrollbar_v(&self, renderer: &mut dyn Renderer, vp: &Rect, content_h: f32, scroll_y: f32, _theme: &Theme) {
        let Some(thumb) = self.thumb_rect_v(vp, content_h, scroll_y) else { return; };
        let track_x = vp.x + vp.width - SCROLLBAR_W - SCROLLBAR_PAD;
        let track_y = vp.y + SCROLLBAR_PAD;
        let track_h = vp.height - SCROLLBAR_PAD * 2.0;
        ...
        renderer.fill_rect(
            ...
            Color { r: 1.0, g: 1.0, b: 1.0, a: 0.05 },
            Corners::all(SCROLLBAR_W / 2.0),
        );
        renderer.fill_rect(
            thumb,
            Color { r: 1.0, g: 1.0, b: 1.0, a: if self.hovered { 0.35 } else { 0.2 } },
            Corners::all(SCROLLBAR_W / 2.0),
        );
    }

    fn draw_scrollbar_h(&self, renderer: &mut dyn Renderer, vp: &Rect, content_w: f32, scroll_x: f32, _theme: &Theme) {
        let Some(thumb) = self.thumb_rect_h(vp, content_w, scroll_x) else { return; };
        let track_x = vp.x + SCROLLBAR_PAD;
        let track_y = vp.y + vp.height - SCROLLBAR_W - SCROLLBAR_PAD;
        let track_w = vp.width - SCROLLBAR_PAD * 2.0 - SCROLLBAR_W - SCROLLBAR_PAD;
...
impl Widget for ScrollArea {
    ...
    fn draw(&self, renderer: &mut dyn Renderer, bounds: Rect, theme: &Theme) {
        self.last_bounds.set((bounds.x, bounds.y, bounds.width, bounds.height));
    ...
    fn handle_event(&mut self, event: &Event, _bounds: Rect) -> EventStatus {
        ...
        let bounds       = if lw > 0.0 { Rect::new(lx, ly, lw, lh) } else { _bounds };
        let vp           = self.viewport(bounds);
        ...
        match event {
            Event::MouseMove { pos } => {
                ...
                if self.hovered {
                    let adj = Event::MouseMove { pos: Point::new(pos.x + sx, pos.y + sy) };
                    let child_bounds = Rect::new(vp.x, vp.y, cw.max(vp.width), ch.max(vp.height));
    ...
    fn intrinsic_size(&self, _theme: &Theme) -> (f32, f32) {
        ...
        (self.max_width.unwrap_or(0.0), self.max_height.unwrap_or(0.0))
    }
}
